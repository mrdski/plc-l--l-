...
public class GeneratorTests {
    ...
    @ParameterizedTest(name = "{0}")
    @MethodSource("testSourceExtra")
    void testSourceExtra(String test, Ast.Source ast, String expected) {
        test(ast, expected);
    }

    private static Stream<Arguments> testSourceExtra() {
        return Stream.of(
                Arguments.of("Hello, World!",
                        // FUN main(): Integer DO
                        //     print("Hello, World!");
    ...
    @ParameterizedTest(name = "{0}")
    @MethodSource("testGlobalExtra")
    void testGlobalExtra(String test, Ast.Global ast, String expected) {
        test(ast, expected);
    }

    private static Stream<Arguments> testGlobalExtra() {
        return Stream.of(
                Arguments.of("List Initialization",
                        new Ast.Global(
                                "nums", "Integer", true,
                                Optional.of(new Ast.Expression.PlcList(
                                        Arrays.asList(
                ...
                Arguments.of("Immutable Initialization",
                        new Ast.Global(
                                "name", "Decimal", false,
                                Optional.of(new Ast.Expression.Literal(BigDecimal.ONE))
                        ),
                        "final double name = 1;"
                )
        );
    ...
    void testListExtra() {
        // LIST list: Decimal = [1.0, 1.5, 2.0];
        Ast.Expression.Literal expr1 = new Ast.Expression.Literal(new BigDecimal("1"));
        Ast.Expression.Literal expr2 = new Ast.Expression.Literal(new BigDecimal("2"));
        Ast.Expression.Literal expr3 = new Ast.Expression.Literal(new BigDecimal("3"));
        expr1.setType(Environment.Type.DECIMAL);
        expr2.setType(Environment.Type.DECIMAL);
        expr3.setType(Environment.Type.DECIMAL);

        Ast.Global global = new Ast.Global("list", "Integer", false, Optional.of(new Ast.Expression.PlcList(Arrays.asList(expr1, expr2, expr3))));
        Ast.Global astList = init(global, ast -> ast.setVariable(new Environment.Variable("list", "list", Environment.Type.INTEGER, false, Environment.create(Arrays.asList(new Integer(1), new Integer(2), new Integer(3))))));

        String expected = new String("final int[] list = {1, 2, 3};");
        test(astList, expected);
    }

    @ParameterizedTest(name = "{0}")
    @MethodSource("testEmptyWhileStatement")
    void testEmptyWhileStatement(String test, Ast.Statement.While ast, String expected) {
        test(ast, expected);
    }
    private static Stream<Arguments> testEmptyWhileStatement() {
        return Stream.of(
                Arguments.of("Empty While Statement",
